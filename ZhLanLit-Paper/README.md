...
### 1. Paper 1 标准六段式架构 (4 部分 / 6 段，1000–1400字)
- **第 1 段**：开头段（明确体裁与时代语境，提出中心论点 Thesis，引出三维互斥分论点）。
- **第 2–4 段**：主体分析段（分别对应视角语态、修辞意象、篇章结构三维度，严格践行 **PEEL 微循环**：微观嵌入引文 $\to$ 手法机制解构 $\to$ 审美效果与主旨升华）。
- **第 5 段**：**辩证补充段（Level 7 核心）**（以“固然……然而……”句式剖析手法内在表现力代价与盲区，深化思辨张力）。
- **第 6 段**：结尾段（综合三维论析与辩证思考，升华至形而上哲学与人性探索）。

### 2. Paper 2 标准八段式架构 (4 部分 / 8 段，1200–1600字)
...
```
ZhLanLit-Paper/
├── SKILL.md                          # 核心指令、双模式路由判定、字数与段落蓝图
├── README.md                         # [本文件] 中文使用指南与提示词手册
├── evals/
│   ├── evals.json                    # 3 组完整任务的端到端评测集与严格断言（Assertions）
│   └── trigger_eval.json             # 20 组触发率分类测试集（10 True / 10 False）
└── references/
    ├── README.md                     # 知识库索引与导航指南
    ├── paper1/                       # 卷一（Paper 1）单文本分析专项
    │   ├── framework.md              # 方法论：三维互斥分类学、PEEL细读模型与学术语体
    │   ├── templates.md              # 模板库：标准 4 部分 / 6 段式模板与高阶句式库
    │   ├── exemplars.md              # 范文库：文学散文与非文学专栏两篇 20/20 满分范文及考官解析
    │   └── research.md               # 研报库：官方评分标准、分级描述与备考策略报告
    └── paper2/                       # 卷二（Paper 2）双作品比较学术论文专项
        ├── framework.md              # 方法论：双维度交叉对比模型与求同析异四维归因指南
        ├── templates.md              # 模板库：标准 4 部分 / 8 段式比较论文模板与句式库
        ├── exemplars.md              # 范文库：余华《活着》vs古华《芙蓉镇》20/20 满分范文及逐段解析
        └── research.md               # 研报库：官方评分标准（Criteria A-D, B2核心）、配对矩阵与时间管理
```
